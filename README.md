# Hi, I'm Aleksandar Todorovic

I build React and Firebase applications focused on product clarity, practical UX, and clean engineering.

My work is centered around real app flows: routing, state, authentication, dashboards, responsive interfaces, Firebase-backed data, and user-facing product polish.

---

## Featured projects

### LifeRecompiled

A production-style React/Firebase engineering case study built around a community/blog app surface.

The project includes posts, comments, reactions, saved posts, profiles, dashboard flows, moderation/reporting, Firebase Auth, Firestore, Cloud Functions, and resilient UX patterns.

**What it demonstrates:**

* React + Firebase full-stack architecture
* Firebase Auth and protected routes
* Firestore data modeling
* Cloud Functions v2
* Cloud Functions-maintained aggregates
* Firestore security rules
* saved-post resilience
* moderation/reporting MVP
* practical UX systems and edge-case handling

**Live:** https://liferecompiled.com
**Repo:** https://github.com/aleksandar-todorovic-dev/liferecompiled

---

### Training App

A mobile-first, local-first training MVP for structured workout cycles, guided logging, previous-value continuity, and educational training support.

A product-focused project built around structured user flow, real-life training interruptions, guided workout execution, and local-first MVP decisions.

**What it demonstrates:**

* React app architecture
* mobile-first UI
* route-based screen flow
* Context + reducer state
* localStorage persistence
* guided workout flow
* previous-value continuity
* partial-day support
* product thinking and MVP scope control

**Repo:** https://github.com/aleksandar-todorovic-dev/training-app

---

## Core stack

* React
* JavaScript
* Vite
* React Router
* Tailwind CSS
* Firebase Auth
* Firestore
* Cloud Functions
* Cloudinary
* GitHub Actions

---

## Also worked with

* Redux Toolkit
* Framer Motion
* Recharts
* React Toastify
* LocalStorage-based persistence
* REST/API data fetching
* vanilla JavaScript, HTML, and CSS

---

## What I can help build

* React frontends and MVPs
* responsive websites and app interfaces
* dashboards and admin panels
* Firebase-backed web apps
* authentication flows
* CRUD and data-driven interfaces
* product UI cleanup and polish
* Figma-to-React implementation
* technical README and project documentation polish

---

## Supporting projects

Alongside my two flagship projects, I keep smaller JavaScript and React projects that show my learning path and breadth:

* weather/API apps
* CRUD and LocalStorage apps
* quote/favorites apps
* movie search/detail flows
* schedule and task management apps
* ecommerce/cart UI practice
* Redux Toolkit practice

These projects are smaller, but they show the path from JavaScript fundamentals to larger React/Firebase applications.

---

## Current focus

* building practical React and Firebase applications with clear UX and maintainable structure
* strengthening TypeScript and Node.js fundamentals
* improving full-stack thinking through auth, data modeling, backend logic, and deployment workflows
